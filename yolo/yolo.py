...
from utils import get_labels_from_txt, letterbox, scale_coords, nms, draw_bbox  # 模型前后处理相关函数


class Model(ABC):
    def __init__(self, model_path):
        print(f"load model {model_path}")
        self.SUCCESS = 0  # 成功状态值
        self.FAILED = 1  # 失败状态值
        self.ACL_MEM_MALLOC_NORMAL_ONLY = 2  # 申请内存策略, 仅申请普通页
    ...
    def _gen_output_dataset(self):
        ...
        for i in range(self._output_num):
            ...
            temp_buffer, ret = acl.rt.malloc(temp_buffer_size, self.ACL_MEM_MALLOC_NORMAL_ONLY)  # 为每个输出申请device内存
...
class YoloV5(Model):
    ...
    def infer(self, img_bgr):
        labels_dict = get_labels_from_txt('./model/coco_names.txt')  # 得到类别信息，返回序号与类别对应的字典
    ...
    def deinit_acl(self, context, device_id):
    ...
    def detect(self, image_path):
        context = self.init_acl(0)  # 初始化acl相关资源
        det_model = YoloV5(model_path='./model/yolov5s_bs1.om')  # 初始化模型
        ...
        # 数据前处理
        img2, scale_ratio, pad_size = letterbox(img, new_shape=[640, 640])  # 对图像进行缩放与填充
        img2 = img2[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, HWC to CHW
        img2 = np.ascontiguousarray(img2, dtype=np.float32) / 255.0  # 转换为内存连续存储的数组

        # 模型推理, 得到模型输出
        output = self.execute([img2, ])[0]

        # 释放相关资源
        det_model.release()  # 释放 acl 模型相关资源, 包括输入数据、输出数据、模型等
        self.deinit_acl(context, 0)  # acl 去初始化
